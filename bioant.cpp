...
void init_menu_principal()
{
	...
	menu[0][1].descricao="Configuracao";
	menu[0][1].codigo="PA";
	...
	menu[3][1].descricao="Inicializar Ambiente";
	menu[3][1].codigo="IA";
	...
	menu[4][0].descricao="LOG";

	menu[4][1].descricao="Mostrar";
	menu[4][1].codigo="ML";
	menu[4][2].descricao="Inicializar";
	menu[4][2].codigo="ZL";

	menu[5][0].descricao="AVL";
	
	menu[5][1].descricao="Inicializar";
	menu[5][1].codigo="RA";

	menu[6][0].descricao="Ajuda";
	
	menu[6][1].descricao="Sobre";
	menu[6][1].codigo="SO";
...
void clone_brains()	// clonagem para TODOS os 100, mesmo nao configurados
{
	int i;
...
{
	if(parametros.avaliacao_ligado)
...
void altera_conjunto_atual()
{
	...
	center(55,"         [CurRight][CurLeft] MUDAR VALOR, [Esc] SAIR               ");
...
void gerencia_sequencia_atual()
{
	...
	while(key!=char(27))
	{
		...
		center(54,"         [CurDown] LE PROXIMA SEQUENCIA, [1] LE PRIMEIRA SEQUENCIA,");

		center(55,"         [d] INICIALIZAR SEQUENCIA, [u] INICIALIZAR CONJUNTO,      ");
		
		center(56,"         [Enter] ALTERA CONJUNTO, [g] GRAVAR SEQUENCIA, [esc] SAIR ");
...
void avaliar_aprendizado()
{
	...
	fim_sequencias=false;

	while(fim_sequencias==false && esc!=char(27))
	{
		passos++;
		...
		if(le_sequencia_atual()==false)
			fim_sequencias=true;

		if(fim_sequencias==false)
		{
...
void devolve_cor_fases(int dec)
{
	...
	case code_saia_do_ninho 		: setcolor( XFOREGROUND_WHITE | XBACKGROUND_LIGHTRED ); break; 
	case code_explore_ambiente 		: setcolor( XFOREGROUND_WHITE | XBACKGROUND_LIGHTRED ); break; 
	case code_segue_cheiro_feromonio : setcolor( XFOREGROUND_WHITE | XBACKGROUND_LIGHTRED ); break; 
	case code_segue_cheiro_alimento : setcolor( XFOREGROUND_WHITE | XBACKGROUND_LIGHTRED ); break; 
	case code_pegue_alimento 		: setcolor( XFOREGROUND_WHITE | XBACKGROUND_LIGHTRED ); break; 
	case code_ache_cheiro_ninho 	: setcolor( XFOREGROUND_WHITE | XBACKGROUND_LIGHTGREEN ); break; 
	case code_segue_cheiro_ninho 	: setcolor( XFOREGROUND_WHITE | XBACKGROUND_LIGHTGREEN ); break; 
	case code_entre_no_ninho 		: setcolor( XFOREGROUND_BLACK | XBACKGROUND_LIGHTGREEN ); break; 
	case code_alertar_perigo 		: setcolor( XFOREGROUND_WHITE | XBACKGROUND_DARKGRAY ); break; 
...
void legenda(bool modo_debug)
{
	...
	setcolor( XFOREGROUND_WHITE | XBACKGROUND_LIGHTRED );
	...
	if(modo_debug)
	{
		...
		center(56," TECLAS: [CurUp][CurDown] ESCOLHER FORMIGA, [Enter] VISUALIZAR DETALHES,        ");
	    center(57," [0-4] NIVEL, DADOS: [a] ALOMONIO, [f] FEROMONIO, [n] NINHO, [c] ALIMENTO,      ");
		center(58," [b] BARULHO, [m] VISUALIZACAO NORMAL, [o] BOOM! [-] SILENCIO, [esc] SAIR       ");
...
void mostra_sentidos(int index_formiga)
{
	...
	if(formiga[index_formiga].vida==viva && formiga[index_formiga].no_ninho==false)
	{
		...
		explica_linha(46,d12.linha);

	} else
		if(formiga[index_formiga].vida!=viva)
			center(46,"( + + + + + + + + + + + )"); else
			center(46,"(VERIFIQUE VALOR DA VARIAVEL \"DESCANSO - ITERACOES\")");
...
void simulacao_testar_sensores(int index_formiga)
{
	...
	center(43," ANALISE DO ALGORITMO SIMBOLICO ");
	...
	center(56," [CurUp] PARA FRENTE, [CurDown] PARA TRAS, [CurRight] VIRA SENTIDO HORARIO,     ");
	center(57," [CurLeft] VIRA SENTIDO CONTRA HORARIO, [a] LIBERA ALOMONIO, [f] LIBERA         ");
	center(58," FEROMONIO, [v] MUDA ESTADO DE VIDA, [s] SAIA DO NINHO,          [esc] SAIR     ");
...
{
	// MUNDO VIRTUAL
	parametros.numero_formigas=40;
	parametros.degradacao_feromonio=20;
	parametros.degradacao_alomonio=5;
	...
	parametros.limite_generec=2000;
	...
	parametros.avaliacao_ligado=false;
...
void mostra_janela_parametros()
{
	...
	center(16,"CONFIGURACAO DO SISTEMA");
	...
	gotoxy(8,47);
	printf("Geracao de Dados de Avaliacao->");
	...
	center(49," TECLAS: [CurUp][CurDown] SELECIONAR ITEM, [CurLeft][CurRight] "); 
    center(50,"         ALTERAR VALOR, [esc] SAIR                             ");
}
...
void mostra_valor_parametro(bool ligado)
{
	...
	case 17: if(parametros.log_ligado) printf("ligado      "); else
				 printf("desligado     "); break;
	case 18: if(parametros.avaliacao_ligado) printf("ligado      "); else
				 printf("desligado     "); break;
	}
...
void aumenta_parametro()
{
	...
	case 18: parametros.avaliacao_ligado=!parametros.avaliacao_ligado; return;
	}
...
void bioant_parametros()
{
	...
	while(escolha!=char(27))
	{
		...
		case char(72): if(parametros.posicao_cursor>0)
						...
						break;
		case char(80): if(parametros.posicao_cursor<18)
						   parametros.posicao_cursor++;
						break;
	...
	for(index_formiga=0;index_formiga<parametros.numero_formigas;index_formiga++)
	{
			...
			//(formiga[index_formiga].alerta || alerta_ligado==false))	// atividades das formigas
		{
			...
			case code_escape 				:	//formiga[index_formiga].repeticao=0;
												...
												{
													fprintf(debug,"<REDE> +++ +++ +++ +++ +++ +++ +++ +++ +++\n");
													fprintf(debug,"<REDE> ERRO NO NUMERO DE ENTRADAS (%i)\n",conta_entradas_positivas(index_formiga)); 
													dump_curto_entradas_saidas(index_formiga);
													fprintf(debug,"<REDE> +++ +++ +++ +++ +++ +++ +++ +++ +++\n");
												}
...
void simulacao_executar()
{
	...
	if(parametros.avaliacao_ligado)
	{
		fprintf(aval,"------ NOVA AVALIACAO ------\n");
		if(parametros.modo_neural)
			fprintf(aval,"MODO: ALGORITMO HIBRIDO\n"); else
			fprintf(aval,"MODO: ALGORITMO SIMBOLICO\n");
	}
...
void ajuda_sobre()
{
	...
	center(28,"Aluno: Marvin Oliver Schneider ");

	center(30,"Orientador: Prof. Dr. Joao Luis Garcia Rosa  ");

	center(32,"Enderecos para contato:");

	center(34,"marvin.schneider@gedas.com.br (comercial)");

	center(36,"moschneider@ig.com.br (particular)");

	center(38,"Campinas, Novembro de 2003");
...
void macro_entre_no_ninho(int index_formiga)
{
		...
		entrar_ninho(index_formiga,false); else
	{
		buscar_sinal(index_formiga);
		ir_para_frente(index_formiga,false);
		formiga[index_formiga].macro_code=code_flush;	// erro grotesco, entao FLUSH!!!
	}
...
//	teste_gerador();
	...
	caixa_mensagem(aviso,0,12,"Favor maximizar a janela para melhor visualizacao (80 x 60)!");
...
//	rede_testar_aprendizado();
	...
	while(codigo_extraido!="XX" && codigo_extraido!="ZZ")
	{
		...
		if(codigo_extraido=="RA")
		{
			limpa_dropdown();
			if(caixa_mensagem(pergunta,0,15,"Zerar o arquivo BIOANT.AVL?"))
				deleta_arquivo_avl();
			restore();
			dropdown();
		}
	...
	clrscr( XFOREGROUND_LIGHTGRAY | XBACKGROUND_BLACK );
